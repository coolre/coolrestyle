(function($) {
	$.fn.nbspSlider = function(options){
		...
		return this.each(function() {  
			...
			var end_index = len-1;		
			var start_index = 0;
			options.effect == "horizontal" ? $(obj).find("ul").css('width',(len*parseInt(options.widths))+'px')
			: $("ul",obj).css('width',options.widths);
			$(obj).css({width:options.widths,height:options.heights,position:"relative",overflow:'hidden'});
			$("ul",obj).css({position:"absolute",left:"0"});
			$("ul li,img",obj).css({width:options.widths,height:options.heights,border:"none",float:"left"});
			$(obj).append('<span class="sliderBtn" id='+options.prevId+'></span><span class="sliderBtn" id='+options.nextId+'></span><div id="altbox">'+$(obj).find("ul li img").eq(0).attr("alt")+'</div><div id="btnList"></div>');
			$("#altbox",obj).css({'width':(parseInt(options.widths)-20)+'px','height':options.altHeight,'line-height':options.altHeight,'opacity':options.altOpa,'background':options.altBgColor,'padding':'0 10px 0 10px','position':'absolute','bottom':'0','color':options.altFontColor,'text-align':'left','left':'0'});
			var $btnlens = len;
			for(var $i=1; $i<=$btnlens; $btnlens--){
				$(obj).find("#btnList").append("<li>"+(options.numBtnSty == "num" ? $btnlens : "<i style='float:left;text-indent:-9999px;'>"+$btnlens+"</i>")+"</li>")	
			}
			$("#btnList",obj).css({'width':(parseInt(options.widths)-20)+'px','left':'0'});
			
			options.numBtnSty == "num" ? $("#btnList li",obj).addClass("btnSty") : $("#btnList li",obj).addClass(options.numBtnSty);
			...
			$("ul",obj).bind("fn_"+options.effect,function(event,extent){
				options.effect == "horizontal" ? options.noEffect == 1 ? $(this).css("margin-left",extent) : ((start_index == 0 || start_index == end_index) && options.starEndNoEff == 1 ) ? $(this).css("margin-left",extent) : $(this).animate({marginLeft:extent},options.speeds) : (options.effect == "none" || (options.starEndNoEff == 1 && (start_index == 0 || start_index == end_index))) ? $(this).css("margin-top",extent) : options.effect == "fade" ? $(this).hide(1).animate({marginTop:extent},30).fadeIn("slow") : $(this).animate({marginTop:extent},options.speeds);
				AltFun(start_index);
			});
